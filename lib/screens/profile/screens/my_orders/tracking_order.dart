...
class _TrackingOrderScreenState extends State<TrackingOrderScreen> {
  ...
  int _currentStatusIndex() {
    if (orderDetails == null) {
      return -1; // Return -1 or any other value indicating that the orderDetails is null.
    }
    return _orderStatus.indexWhere(
        (status) => status['status'] == orderDetails!.currentStatus);
  }

  String _calculateExpectedDeliveryDate() {
    if (orderDetails == null) {
      return 'N/A'; // Return 'N/A' or any other placeholder value indicating that the orderDetails is null.
    }

    final DateTime orderDate = orderDetails!.orderDate;
  ...
  String _getChangeTime(String status) {
    if (orderDetails?.statusHistory == null) {
      return 'N/A';
    }

    final changeTimeString = orderDetails!.statusHistory;
    ...
    if (changeTimeMap.containsKey(status)) {
      try {
        final dateTime = DateTime.parse(changeTimeMap[status]!);
        return DateFormat('dd MMM yyyy, hh:mm a').format(dateTime);
      } catch (e) {
        print('Error parsing date for status $status: $e');
        return 'N/A';
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<OrderCubit, OrderState>(
      listener: (context, state) {
        if (state is FetchOrderDetailsSuccess) {
          setState(() {
                ...
                child: Column(
                  ...
                  children: [
                              ...
                              slivers: [
                                SliverList(
                                  delegate: SliverChildBuilderDelegate(
                                    (context, index) {
                                      final item = orderDetails!.details[index];
                                      return OrderItem(
                                        imageUrl: item.product.imageUrl,
                                        title: item.product.productName,
                                        size: item.product.size.toString(),
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('Order ID', style: textTheme.displayMedium),
