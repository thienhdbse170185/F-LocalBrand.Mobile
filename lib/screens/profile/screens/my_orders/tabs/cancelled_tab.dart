...
class _CancelledTabState extends State<CancelledTab> {
  ...
  Future<void> _fetchOrders() async {
    ...
    context.read<OrderCubit>().fetchCancelledOrderHistory(
        customerId); // Ensure the method exists in the OrderCubit
  }
  ...
  Widget build(BuildContext context) {
    return BlocListener<OrderCubit, OrderState>(
      listener: (context, state) {
        if (state is FetchCancelledOrderHistoryInprogress) {
          setState(() {
            _isLoading = true;
          });
        } else if (state is FetchCancelledOrderHistorySuccess) {
          setState(() {
            orders = state.orderTrackingList;
            _isLoading = false;
            _isEmpty = orders!.isEmpty; // Check if orders list is empty
          });
        } else if (state is FetchCancelledOrderHistoryFail) {
          setState(() {
            _isLoading = false;
            _isEmpty = true;
          });
